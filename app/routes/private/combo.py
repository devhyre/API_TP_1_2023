...
async def create_combo(combo: ComboPost, db: Session = Depends(get_db), user: dict = Depends(get_current_active_user)):
    ...
    lista_productos = [combo.case_id, combo.motherboard_id, combo.procesador_id, combo.ram_id,
                       combo.almacenamiento_id, combo.cooler_id, combo.gpu_id, combo.fan_id, combo.fuente_id]
    ...
    lista_cantidades = [combo.quantity_case, combo.quantity_motherboard, combo.quantity_procesador, combo.quantity_ram,
                        combo.quantity_almacenamiento, combo.quantity_cooler, combo.quantity_gpu, combo.quantity_fan, combo.quantity_fuente]
    for i in range(len(lista_productos)):
        if lista_productos[i] is None:
            continue
        if lista_cantidades[i] is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail='La cantidad de productos debe ser mayor a 0')
    # Crear ID de combo que sera compuesto por COM+CATEGORIA+ID
    number_random = str(datetime.now().year) + str(datetime.now().month) + str(datetime.now().day) + \
    ...
    combo_db = ComboModel(
        ...
        quantity_case=combo.quantity_case,
        motherboard_id=combo.motherboard_id,
        quantity_motherboard=combo.quantity_motherboard,
        procesador_id=combo.procesador_id,
        quantity_procesador=combo.quantity_procesador,
        ram_id=combo.ram_id,
        quantity_ram=combo.quantity_ram,
        almacenamiento_id=combo.almacenamiento_id,
        quantity_almacenamiento=combo.quantity_almacenamiento,
        cooler_id=combo.cooler_id,
        quantity_cooler=combo.quantity_cooler,
        gpu_id=combo.gpu_id,
        quantity_gpu=combo.quantity_gpu,
        fan_id=combo.fan_id,
        quantity_fan=combo.quantity_fan,
        fuente_id=combo.fuente_id,
        quantity_fuente=combo.quantity_fuente,
...
async def delete_combo(combo_id: str, db: Session = Depends(get_db), user: dict = Depends(get_current_active_user)):
    ...
    if not combo_db:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail='Combo no encontrado')
