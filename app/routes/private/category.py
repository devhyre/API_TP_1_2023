...
async def registrar_categoria(description: str, db: Session = Depends(get_db), user: ProfileResponse = Depends(get_current_active_user)):
    user_type = list(user.keys())[0]
    if user_type == 'client':
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='No tiene permisos para realizar esta acción')
    else:
        category = db.query(TableOfTablesModel).filter(TableOfTablesModel.id == 3 and TableOfTablesModel.description == description).first()
        if category:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"La categoria con descripción {description} ya existe")
